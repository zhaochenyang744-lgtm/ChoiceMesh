...
export default function ChoiceMeshApp() {
  ...
  async function withdrawProposal(id: string) {
    const { error: withdrawError } = await supabase.rpc("withdraw_proposal", { p_proposal_id: id });
    if (withdrawError) { setError(withdrawError.message); return; }
    setToast("Change withdrawn. The current proposal remains unchanged.");
    if (roomId) await loadRoom(roomId);
  }
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {room && activePage === "now" && <NowPage room={room} proposal={proposal} summary={summary} privateDetail={privateDetail} pendingProposals={pendingProposals} currentUserId={user.id} onInvite={copyInvite} onDetails={() => setPage("details")} onChange={() => setChangeOpen(true)} onSupport={supportProposal} onWithdraw={withdrawProposal} />}
      {room && activePage === "details" && <DetailsPage detail={privateDetail} reply={reply} draft={draft} listening={listening} analysing={analysing} saving={saving} onReply={setReply} onAnalyse={analyseReply} onManual={() => { setError(""); setDraft({ ...emptyDraft }); setToast("Manual entry is ready. Fill in only what you want to confirm."); }} onVoice={startVoiceInput} onDraft={setDraft} onSave={saveDetails} onEdit={() => { setPrivateDetail(null); setDraft(privateDetail?.parsed_detail ?? null); setReply(privateDetail?.original_reply ?? ""); }} />}
      ...
      {changeOpen && <ChangeModal date={changeDate} time={changeTime} title={changeTitle} note={changeNote} saving={saving} onClose={closeChange} onDate={setChangeDate} onTime={setChangeTime} onTitle={setChangeTitle} onNote={setChangeNote} onSubmit={submitChange} />}
      {(toast || error) && <div className={error ? "toast error" : "toast"} role={error ? "alert" : "status"}>{error || toast}</div>}
    </section>
  </main>;
}
...
function NowPage({ room, proposal, summary, privateDetail, pendingProposals, currentUserId, onInvite, onDetails, onChange, onSupport, onWithdraw }: { room: Room; proposal: Proposal | null; summary: Summary; privateDetail: PrivateDetail | null; pendingProposals: Proposal[]; currentUserId: string; onInvite: () => void; onDetails: () => void; onChange: () => void; onSupport: (id: string) => void; onWithdraw: (id: string) => void }) {
  const statusText = pendingProposals.length > 0
    ? `${pendingProposals.length} proposal ${pendingProposals.length === 1 ? "change" : "changes"} to resolve`
    : summary.can_publish
      ? "Ready to publish"
      : `${Math.max(0, room.minimum_confirmations - summary.confirmed_count)} more ${room.minimum_confirmations - summary.confirmed_count === 1 ? "confirmation" : "confirmations"} needed`;
  return <section className="page"><div className="topline"><div><p className="eyebrow">CHOICEMESH · NOW</p><h1>What is still open?</h1></div><div className="top-actions"><button className="button ghost" onClick={onInvite}>Copy invite link</button><button className="button ghost" onClick={onDetails}>My details</button></div></div><div className="now-grid"><section className="surface plan-card"><div className="section-row"><div><h2>{proposal?.title || "Proposal loading"}</h2><p>{formatWhen(proposal?.starts_at || room.starts_at)} · {room.minimum_confirmations} confirmations needed</p></div><span className={summary.can_publish ? "status green" : "status amber"}>{statusText}</span></div><div className="flow"><span><b>Individual details</b><small>Private to each member</small></span><i /><span className="right"><b>Shared proposal</b><small>{proposal?.title || "Loading"}</small></span></div>{!privateDetail?.confirmed_at && <Task icon="□" title="Confirm your private details" copy="Your own words are analysed privately, then you review the draft before saving." action="My details" onClick={onDetails} />}{privateDetail?.confirmed_at && summary.response_count < room.minimum_confirmations && <Task icon="•" title="The group is still waiting for responses" copy="A reminder never submits or confirms details for another person." action="Copy invite" onClick={onInvite} />}{pendingProposals.length > 0 && <div className="pending-list"><b>Proposal changes waiting for resolution</b>{pendingProposals.map((item) => <div key={item.id} className="pending-item"><span>{item.title}</span>{item.created_by === currentUserId ? <button className="button ghost" onClick={() => onWithdraw(item.id)}>Withdraw change</button> : <button className="button ghost" onClick={() => onSupport(item.id)}>Support change</button>}</div>)}</div>}<div className="proposal-link"><button className="text-button" onClick={onChange}>Propose a change</button><span>A change does not replace the current proposal until another member supports it.</span></div></section><aside className="now-side"><section className="surface metric"><h2>Response progress</h2><strong>{summary.response_count}<sup>/{room.minimum_confirmations}</sup></strong><p>{summary.confirmed_count} members are confirmed attending</p></section><section className="surface awareness"><h2>For awareness</h2><p><span>!</span>{summary.boundary_risk_count ? `${summary.boundary_risk_count} confirmed boundary risk${summary.boundary_risk_count === 1 ? " remains" : "s remain"} visible. ${summary.boundary_risk_count === 1 ? "It does" : "They do"} not require a follow-up by default.` : "No confirmed boundary risks yet."}</p></section></aside></div></section>;
}

function DetailsPage({ detail, reply, draft, listening, analysing, saving, onReply, onAnalyse, onManual, onVoice, onDraft, onSave, onEdit }: { detail: PrivateDetail | null; reply: string; draft: DetailDraft | null; listening: boolean; analysing: boolean; saving: boolean; onReply: (value: string) => void; onAnalyse: () => void; onManual: () => void; onVoice: () => void; onDraft: (draft: DetailDraft | null) => void; onSave: () => void; onEdit: () => void }) {
  const confirmed = Boolean(detail?.confirmed_at);
  return <section className="page page-wide"><div className="page-intro compact"><p className="eyebrow">MY DETAILS · PRIVATE</p><h1>My details</h1><p>Your details are shared only as an anonymous status after you confirm them.</p></div>{confirmed ? <section className="surface confirmed-details"><div className="section-row"><div><h2>Your details are confirmed</h2><p>You can update them if your situation changes.</p></div><span className="status green">Confirmed</span></div><dl className="detail-grid"><div><dt>Attendance</dt><dd>{detail?.attendance.replace("_", " ")}</dd></div><div><dt>Travel limit</dt><dd>{detail?.parsed_detail.travel_limit_minutes ? `${detail.parsed_detail.travel_limit_minutes} minutes` : "Not specified"}</dd></div><div><dt>Budget limit</dt><dd>{detail?.parsed_detail.budget_limit ? `$${detail.parsed_detail.budget_limit}` : "Not specified"}</dd></div><div><dt>Shared view</dt><dd>Status only</dd></div></dl><button className="button ghost" onClick={onEdit}>Update my details</button></section> : <section className="form-surface details-entry"><div className="form-heading"><h2>What works for you?</h2><p>Speak or type naturally. ChoiceMesh makes a private draft for you to check.</p></div><label className="input-label" htmlFor="reply">Your reply</label><div className="reply-wrap"><textarea id="reply" value={reply} onChange={(event) => onReply(event.target.value)} placeholder="For example: I can probably make Sunday, but public transport over 45 minutes will not work. I can confirm by Friday." /><button type="button" className={listening ? "mic listening" : "mic"} onClick={onVoice} aria-label="Voice input" title="Voice input">●<span>{listening ? "Listening" : "Voice input"}</span></button></div><div className="entry-actions"><button className="button primary" onClick={onAnalyse} disabled={analysing}>{analysing ? "Analysing…" : "Analyse my reply"}</button><button className="button ghost" type="button" onClick={onManual}>Enter details manually</button><small>AI is optional. Your original reply and draft never appear in the shared room.</small></div>{draft && <PrivateReview draft={draft} onChange={onDraft} onConfirm={onSave} saving={saving} />}</section>}</section>;
}
...
function PublishPage({ room, proposal, summary, ownConfirmed, pendingCount, saving, onPublish }: { room: Room; proposal: Proposal | null; summary: Summary; ownConfirmed: boolean; pendingCount: number; saving: boolean; onPublish: () => void }) {
  ...
  const blockedBy = !ownConfirmed
    ? { title: "Confirm your details", copy: "Review and confirm your own private details before publishing.", button: "Your confirmation is needed" }
    : pendingCount > 0
      ? { title: "Resolve the proposal change", copy: "Support the pending change or ask its proposer to withdraw it before publishing.", button: "Proposal change pending" }
      : { title: "Waiting for the group", copy: `${Math.max(0, room.minimum_confirmations - summary.confirmed_count)} more confirmation${room.minimum_confirmations - summary.confirmed_count === 1 ? " is" : "s are"} needed.`, button: "Waiting for confirmations" };
  return <section className="page"><div className="page-intro compact"><p className="eyebrow">PUBLISH CHECK</p><h1>Ready to publish?</h1><p>Publishing makes the current proposal the shared result and notifies everyone.</p></div><div className="publish-grid"><section className="surface checklist"><h2>Before you publish</h2><ChecklistItem ok={ownConfirmed} text="Your private details are confirmed" /><ChecklistItem ok={summary.confirmed_count >= room.minimum_confirmations} text={`${room.minimum_confirmations} members are confirmed attending`} sub={`${summary.confirmed_count} confirmations are recorded.`} /><ChecklistItem ok={pendingCount === 0} text="No proposal change is waiting for support" /><ChecklistItem ok text="Private reasons remain private" sub="Publishing shares the agreed proposal, never anyone’s raw details." /></section><aside className="publish-panel"><p className="eyebrow">READY TO SHARE</p><h2>{ready ? "Make it official?" : blockedBy.title}</h2><p>{ready ? `Publish “${proposal?.title}” as the shared decision.` : blockedBy.copy}</p><button className="button publish-button" disabled={!ready || saving} onClick={onPublish}>{saving ? "Publishing…" : ready ? "Publish decision" : blockedBy.button}</button></aside></div></section>;
}
...
function Progress({ active, published }: { active: Page; published: boolean }) { const steps: Array<[Page, string]> = [["create", "Create"], ["now", "Now"], ["details", "Details"], ["publish", "Publish"], ["result", "Result"]]; const index = steps.findIndex(([id]) => id === active); return <ol className="progress" aria-label="Decision flow">{steps.map(([id, label], stepIndex) => <li key={id} aria-current={id === active ? "step" : undefined} className={id === active ? "active" : stepIndex < index || (id === "result" && published) ? "done" : ""}><i aria-hidden="true" />{label}</li>)}</ol>; }
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="field"><span>{label}</span>{children}</label>; }
function NavButton({ active, disabled, icon, onClick, children }: { active?: boolean; disabled?: boolean; icon: string; onClick: () => void; children: React.ReactNode }) { const label = typeof children === "string" ? children : undefined; return <button aria-label={label} aria-current={active ? "page" : undefined} className={active ? "nav-button active" : "nav-button"} disabled={disabled} onClick={onClick}><span className="icon" aria-hidden="true">{icon}</span><span>{children}</span></button>; }
