...
(async () => {
  ...
  try {
    ...
    await page.getByRole('button', { name: '发送私密提醒' }).click();
    await expectText(page.locator('#overview-notice'), '不会替任何成员提交反馈');
    await expectText(page.locator('#reminder-note'), '30 分钟后可再次提醒');
    if (!await page.locator('#remind-member').isDisabled()) throw new Error('Reminder should enter a visible cooldown state.');
    ...
    await page.locator('#expected-time-input').fill('2026-08-08T18:00');
    await page.getByRole('button', { name: '保存预计时间' }).click();
    await expectText(page.locator('#deadline-status'), '8月8日 18:00');
    ...
    await page.getByRole('button', { name: '候选比较', exact: true }).click();
    await expectText(active(page), '当前没有可推进的其他候选');
